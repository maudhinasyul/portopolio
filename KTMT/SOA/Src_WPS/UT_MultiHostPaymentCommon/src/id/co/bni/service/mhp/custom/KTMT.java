...
import id.co.bni.ibs.IBSBilling;
import id.co.bni.ibs.dao.IBSBillingDAO;
...
public class KTMT {
	...
	throws Exception {
		...
		DataObject hostRes = (DataObject) savedBO.get("HostResponse");
		...
		if(!"00".equals(payRes.getString("responseCode")) )
		{
			...
			Common.log.info("fungsi update ");
			EntityManager em = mhp_db.createEntityManager();
			KtmtBilling ktmt = KtmtBillingDAO.getInstance() .updateKtmtBillingStatus(payRes.getString("performaNo"), em);
			Common.log.info("KTMT updated!");
//			em.joinTransaction();
//			String paymentCode = payRes.get("performaNo").toString();
//			String currency = payRes.get("currency").toString();
//			String responseCode = hostRes.get("responseCode").toString();
//			String responseDesc = hostRes.getString("responseDesc");
//			KtmtBilling ktmt = getKtmtBilling(paymentCode, currency, em);
//			ktmt.setResponseCode(responseCode);
//			ktmt.setResponseDesc(responseDesc);
//			KtmtBillingDAO.getInstance().update(ktmt, em);
	...
	private static String getJournal(String from, HashMap savedBO) {
		String result = "000000";

		DataObject mcpResponse = (DataObject) savedBO.get("HostResponse");
		DataObject request = (DataObject) savedBO.get("Request");
		DataObject reqKtmt = (DataObject) savedBO.get("Request095491");
		if (mcpResponse != null) {
			result = mcpResponse.getString("header/coreJournal");
		}
		else
		{
			result = reqKtmt.getString("header/coreJournal");
		}
		return result;
	...
	private static KtmtBilling getKtmtBilling(String paymentCode,
		...
		private static  KtmtBilling updateKtmtBilling(String performaNo, 
								 EntityManager em) throws MhpException {
		KtmtBilling ktmt = KtmtBillingDAO.getInstance().updateKtmtBillingStatus(performaNo, em);
		Common.log.info("KtmtBilling updated!");
		if (ktmt == null) {
			Common.log.error("KTMT.reinquiry CANNOT update KtmtBilling by performaNo: " + performaNo);
			throw new MhpException("Ktmt_REINQ", "87",
